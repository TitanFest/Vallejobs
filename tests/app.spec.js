...
test.describe("Vallejobs — Tests de interfaz completa", () => {
  test.describe("Página principal", () => {
    ...
    test("Muestra botones de registro y login (no autenticado)", async ({
      page,
    }) => {
      await page.goto("/");
  ...
  test.describe("Registro de usuario", () => {
    ...
    test("Registra un nuevo usuario correctamente", async ({ page }) => {
      ...
      await expect(page.locator(".registro-container")).toContainText(
        /exitosamente/i,
        { timeout: 10000 },
      );
    });
  ...
  test.describe("Inicio de sesión", () => {
    ...
    test("Inicia sesión correctamente y redirige al dashboard", async ({
      page,
    }) => {
      await page.goto("/login");
  ...
  test.describe("Dashboard", () => {
    test("Muestra tarjetas de estadísticas y nombre del usuario", async ({
      page,
    }) => {
      await page.goto("/login");
      await page.locator('input[type="email"]').fill(TEST_EMAIL);
    ...
    test("Tabs de postulaciones y ofertas funcionan", async ({ page }) => {
      ...
      await expect(page.locator(".dashboard-tab.active")).toContainText(
        "Mis ofertas",
      );
      await page
        .locator('.dashboard-tab:has-text("Mis postulaciones")')
        .click();
      await expect(page.locator(".dashboard-tab.active")).toContainText(
        "Mis postulaciones",
      );
  ...
  test.describe("Navegación autenticado", () => {
    test("Menú de perfil se abre, cierra y tiene opciones", async ({
      page,
    }) => {
      await page.goto("/login");
      await page.locator('input[type="email"]').fill(TEST_EMAIL);
  ...
  test.describe("Rutas protegidas", () => {
    test("Redirige a /Login desde dashboard si no hay sesión", async ({
      page,
    }) => {
    ...
    test("Redirige a /Login desde CreateJob si no hay sesión", async ({
      page,
    }) => {
    ...
    test("Redirige a /Login desde EditProfile si no hay sesión", async ({
      page,
    }) => {
  ...
  test.describe("Búsqueda y filtros", () => {
    ...
    test('Clic en categoría la activa y "Todos" la desactiva', async ({
      page,
    }) => {
      await page.goto("/");
