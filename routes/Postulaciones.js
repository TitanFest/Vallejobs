...
router.post('/postular', authMiddleware, async (req, res) => {
    try {
        ...
        const yaPostulado = await Postulacion.findOne({ where: { userId, ofertaId } });
        if (yaPostulado) return res.status(400).json({ error: 'Ya te postulaste a esta oferta' });
...
router.get('/oferta/:ofertaId', authMiddleware, async (req, res) => {
    try {
        ...
        const oferta = await OfertasTrabajo.findByPk(ofertaId);
        if (!oferta) return res.status(404).json({ error: 'Oferta no encontrada' });
        if (oferta.userId !== userId) return res.status(403).json({ error: 'No autorizado' });

        const postulaciones = await Postulacion.findAll({
            where: { ofertaId },
            include: [{ model: User, as: 'postulante', attributes: ['id', 'name', 'apellido', 'email', 'telefono', 'cv'] }],
        });
        res.json(postulaciones);
    } catch (error) {
...
router.put('/:id/estado', authMiddleware, async (req, res) => {
    try {
        ...
        if (postulacion.oferta.userId !== req.user.userId) return res.status(403).json({ error: 'No autorizado' });
