...
router.put('/actualizar', authMiddleware, upload.fields([{ name: 'foto' }, { name: 'cv' }]), async (req, res) => {
    try {
        const userId = req.user.userId;
        const { name, email, telefono, ubicacion, descripcion } = req.body;

        const [updated] = await User.update(
            { name, email, telefono, ubicacion, descripcion },
            { where: { id: userId } }
        );
