            ...
            {user.cv ? (
              <>
                ...
                <div className="resume-actions">
                  <a href={`http://localhost:5000/${user.cv}`} target="_blank" rel="noreferrer">
                    <button className="view-btn">Ver</button>
                  </a>
                  <button className="update-btn" onClick={() => navigate('/EditProfile')}>
                    Actualizar
                  </button>
                </div>
              </>
        ...
        <div className="ratings-section">
          <div className="rating-card">
            ...
            <div className="stars">
              {[...Array(5)].map((_, i) => (
                <FaStar key={i}
                  className={i < Math.floor(user.rating_empleador || 0) ? 'star-filled' : 'star-empty'} />
              ))}
            </div>
            <span className="rating-value">{user.rating_empleador || 0}/5</span>
          </div>
          <div className="rating-card">
            <h4>Como Empleado</h4>
            <div className="stars">
              {[...Array(5)].map((_, i) => (
                <FaStar key={i}
                  className={i < Math.floor(user.rating_empleado || 0) ? 'star-filled' : 'star-empty'} />
              ))}
            </div>
            <span className="rating-value">{user.rating_empleado || 0}/5</span>
          </div>
        </div>
